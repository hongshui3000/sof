...
#define CORDICCOSINE_TABLE_SIZE 31
...
/* Compute fixed point cordiccosine with table lookup and interpolation
 ...
 *	+------------------+-----------------+--------+--------+
 */
int32_t cos_fixed(int32_t th_rad_fxp)
{
	/*cordic_atan2_lookup_table = atan(2.^-(0:N-1)) N = 31 */
	/*CORDIC Gain is cordic_gain = prod(sqrt(1 + 2.^(-2*(0:31-1)))) */
	/*Inverse CORDIC Gain,inverse_cordic_gain = 1 / cordic_gain */
	static const int32_t inpLUT[CORDICCOSINE_TABLE_SIZE] = { 843314857, 497837829,
	263043837, 133525159, 67021687, 33543516, 16775851, 8388437, 4194283, 2097149,
	1048576, 524288, 262144, 131072, 65536, 32768, 16384, 8192, 4096, 2048, 1024,
	512, 256, 128, 64, 32, 16, 8, 4, 2, 1 };
	int32_t b_idx;
	...
	int sign = 1;
	/* 652032874 , deg = 69.586061*/
	const int32_t CORDICCOSINE_LUT_Q29FL  =	 Q_CONVERT_FLOAT(1.214505869895220, 29);
	/* 421657428 , deg = 90.000000 */
	const int32_t CORDICCOSINE_PIOVERTWO_Q28FL  = Q_CONVERT_FLOAT(M_PI / 2, 28);
	/* 843314857,  deg = 90.000000	*/
	const int32_t CORDICCOSINE_PIOVERTWO_Q29FL  = Q_CONVERT_FLOAT(M_PI / 2, 29);
	/* 1686629713, deg = 90.000000	*/
	const int32_t CORDICCOSINE_PIOVERTWO_Q30FL  = Q_CONVERT_FLOAT(M_PI / 2, 30);
	...
	if (th_rad_fxp > CORDICCOSINE_PIOVERTWO_Q28FL) {
		if ((th_rad_fxp - CORDICCOSINE_PIOVERTWO_Q29FL) <= CORDICCOSINE_PIOVERTWO_Q28FL) {
			th_rad_fxp -= CORDICCOSINE_PIOVERTWO_Q29FL;
			sign  = -1;
		} else {
			th_rad_fxp -= CORDICCOSINE_PIOVERTWO_Q30FL;

		}
	} else if (th_rad_fxp < -CORDICCOSINE_PIOVERTWO_Q28FL) {
		if ((th_rad_fxp + CORDICCOSINE_PIOVERTWO_Q29FL) >= -CORDICCOSINE_PIOVERTWO_Q28FL) {
			th_rad_fxp += CORDICCOSINE_PIOVERTWO_Q29FL;
			sign  = -1;
		} else {
	...
	th_rad_fxp <<= 2;
	b_yn = 0;
	xn = CORDICCOSINE_LUT_Q29FL;
	xtmp = CORDICCOSINE_LUT_Q29FL;
	ytmp = 0;

	for (b_idx = 0; b_idx < CORDICCOSINE_TABLE_SIZE; b_idx++) {
		if (th_rad_fxp < 0) {
